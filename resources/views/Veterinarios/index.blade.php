...
@section('title', 'Veterinarios - Rescate Animales')
	...
	<div class="container-fluid">
		<div class="card card-primary card-outline">
			<div class="card-header d-flex align-items-center">
				...
				<a href="{{ url('/veterinarios/crear') }}" class="btn btn-primary btn-sm ml-2" data-role-allowed="Administrador">
...
(function() {
	...
	function render() {
		...
		const rows = db.get('Veterinario').map(v => ({
			...v,
			...
			return name.includes(q) || (r.especialidad||'').toLowerCase().includes(q);
		});
		$tbody.innerHTML = rows.map(r => `
			<tr>
				<td>${r.veterinario_id}</td>
				<td>${r._persona ? (r._persona.nombre + ' ' + (r._persona.apellido||'')) : '-'}</td>
				<td>${r.especialidad || '-'}</td>
				<td>${r.cv_documentado ? 'Sí' : 'No'}</td>
				<td class="text-right">
					<button class="btn btn-xs btn-info" data-action="edit" data-id="${r.veterinario_id}"><i class="fas fa-pen"></i></button>
					<button class="btn btn-xs btn-danger" data-action="del" data-id="${r.veterinario_id}"><i class="fas fa-trash"></i></button>
				</td>
			</tr>
		`).join('');
	}
	function load(id) {
		const r = db.find('Veterinario', id);
		if (!r) return;
		document.getElementById('id').value = r.veterinario_id;
	...
	function save(e) {
		...
		if (id) db.update('Veterinario', { ...payload, veterinario_id: id });
		else db.create('Veterinario', payload);
		document.getElementById('form').reset();
		render();
	}
	function remove(id) { db.remove('Veterinario', id); render(); }
